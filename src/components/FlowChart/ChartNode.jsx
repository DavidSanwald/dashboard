...
class ChartNode extends React.Component {
  ...
  render = () => {
    ...
    return (
      <div
        className={`chart-node ${isSpecial ? "special" : ""} ${
          properties.replicas ? "stacked" : ""
        } ${isGateway ? "gateway" : ""}`}
      >
        <div className="p-2">
          <p className="m-1">
            <b>{label || <span className="text-warning">Unnamed Pod</span>}</b>
            <Badge variant="primary" className="ml-2 mt-1">
              {properties.replicas}
            </Badge>
          </p>
        </div>
        {icons.length > 0 && <div className="node-icons">{icons}</div>}
      </div>
    )
  }
}
